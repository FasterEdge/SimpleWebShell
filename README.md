...
- 文件上传（multipart，支持指定目标路径）与文件下载（支持中途取消）；单次上传总大小默认上限 2GiB（可用环境变量 `MAX_UPLOAD_SIZE`（字节）调整），表单字段（如 path）上限 64KB，防止异常/恶意请求耗尽磁盘或内存
...
- 自动清理：空闲超过 24 小时的会话会被后台协程清理，会话总数上限 1000（超出时清理最久未访问者），防止长期运行内存无限增长；活跃会话不受影响。
